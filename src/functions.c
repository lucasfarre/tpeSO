...
static PyObject * py_getmemposix(PyObject *self, PyObject *args) {
	char * mem;
	int fd;
	if ( (fd = shm_open("/srvmem7", O_RDWR|O_CREAT, 0666)) == -1 )
...
static PyObject * py_wrlock(PyObject *self, PyObject *args) {
    int fd;
    if (!PyArg_ParseTuple(args, "i", &fd))
        return NULL;
    struct flock lock;
...
static PyObject * py_rdlock(PyObject *self, PyObject *args) {
    int fd;
    if (!PyArg_ParseTuple(args, "i", &fd))
        return NULL;
    struct flock lock;
...
static PyObject * py_rdwrlock(PyObject *self, PyObject *args) {
    int fd;
    if (!PyArg_ParseTuple(args, "i", &fd))
        return NULL;
    struct flock lock;
    ...
    lock.l_len = 0;
    lock.l_pid = getpid();
    lock.l_type = F_RDLCK | F_WRLCK;   /* Test for any lock on any part of file. */
    lock.l_whence = SEEK_SET;
    fcntl(fd, F_SETLKW, &lock);
    return Py_BuildValue("i", 0); // return 0;
}
...
static PyMethodDef Functions[] = {
    ...
    {"wrlock",  py_wrlock, METH_VARARGS, "wrlock"},
    {"rdlock",  py_rdlock, METH_VARARGS, "rdlock"},
    {"rdwrlock",  py_rdwrlock, METH_VARARGS, "rdwrlock"},
